    ...
    <div className="grid min-h-dvh place-items-center bg-hero-gradient px-4 py-8">
      <div className="w-full max-w-md rounded-3xl bg-card p-8 shadow-soft">
        <Link to="/" className="mb-6 inline-flex items-center gap-2">
          <div className="grid h-10 w-10 place-items-center rounded-xl bg-primary-gradient text-primary-foreground font-display font-bold">A</div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {(["patient", "doctor"] as const).map((r) => (
              <button type="button" key={r} onClick={() => setRole(r)}
                className={`rounded-xl border px-4 py-3 text-base font-medium ${role === r ? "border-primary bg-primary-soft text-primary" : "border-border bg-background"}`}>
